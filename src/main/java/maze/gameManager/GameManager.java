...
//        Player player = new Player(playerLocation);
        ...
        for (i = 0; i < data.getMaxSteps(); i++) {
            ...
            if (isWall(currentLocation)) {
                ...
                final Point finalCurrentLocation = currentLocation;
                bookMarksMaps.put(bookmarksCounter, new HashMap<Point, Integer>() {{
                    put(finalCurrentLocation, bookmarksCounter + 1);
                }});
                player.hitBookmark(bookmarksCounter + 1);
